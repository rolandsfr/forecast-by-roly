...
import {transformUnit, getLocationAsText, fetchWeather, formatAMPM, KEYS} from "../../modules/utilities"
...
export default function Home(props) {
    ...
    const [weather, setWeather] = useState({temperature: 0, wind: 0,  humidity: 0, pressure: 0});
    ...
    let settings = JSON.parse(localStorage.getItem("settings")) || {temperature: "Celcius", speed: "m/s", pressure: "mBar"}
    ...
    function fetchForecast(lat, long) {
        if(!signal.aborted) {
            return fetch(`https://api.openweathermap.org/data/2.5/onecall?&exclude=minutely,current&lat=${lat}&lon=${long}&appid=${KEYS.WEATHER_API_KEY}`)
            .then(resp => resp.json())
            .then(res => {
                setForecast(res)
            })
    ...
    function returnHourlyForecast(forecast) {
        ...
        if(forecast) {
            for(let i = 0; i <= 12; i++) {
                ...
                let temperature = transformUnit("temperature",forecast.hourly[i].feels_like, settings.temperature).replace(/[a-zA-Z]/g, "")
    ...
    function returnDailyForecast(forecast) {
        ...
        if(forecast) {
            for(let i = 0; i <= 7; i++) {
                ...
                let dayTemperature = transformUnit("temperature", forecast.daily[i].feels_like.day, settings.temperature).replace(/[a-zA-Z]/g, "")
                let nightTemperature = transformUnit("temperature", forecast.daily[i].feels_like.night, settings.temperature).replace(/[a-zA-Z]/g, "")
    ...
    function getCoords(pos) {
        ...
        getLocationAsText(latitude, longitude, true)
            .then(res => {
                setLocation(res)            
                ...
                fetchWeather(latitude, longitude)
                    .then(weather => {
                        // set the right units
                        let temperature = transformUnit("temperature", weather.main.feels_like, settings.temperature).replace(/[a-zA-Z]/g, "")
                        let wind = transformUnit("speed", weather.wind.speed, settings.speed)
                        let pressure = transformUnit("pressure", weather.main.pressure, settings.pressure)
                        ...
                        setWeather({
                            ...
                            pressure,
                            state: weather.weather[0].description[0].toUpperCase() + weather.weather[0].description.slice(1),
                        })
        ...
        <div id="wrapper">
            ...
            <div className="forecast">
                ...
                <div className="hourly-forecast">
                    {forecast ? <ScrollMenu translate={35} wheel={false} inertiaScrolling={true} inertiaScrollingSlowdown={0.7} data={hourlyForecast}/> : "Loading.."}
                </div>
